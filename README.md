...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create buddy-lore-9c6e
  fly apps create buddy-lore-9c6e-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app buddy-lore-9c6e
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app buddy-lore-9c6e-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'buddy-lore-9c6e-staging' does not match app name in config file 'buddy-lore-9c6e'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name buddy-lore-9c6e-db
  fly postgres attach --app buddy-lore-9c6e buddy-lore-9c6e-db

  fly postgres create --name buddy-lore-9c6e-staging-db
  fly postgres attach --app buddy-lore-9c6e-staging buddy-lore-9c6e-staging-db
  ```
